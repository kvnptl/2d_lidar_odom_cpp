...
std::vector<Vector2d> voxelGridDownsample(const std::vector<Vector2d> &points, double voxel_size)
{
...
int main(int argc, char const *argv[])
{
    ...
    std::vector<Vector2d> source_scan = voxelGridDownsample(pointcloud_set[0], voxel_size);

    for (size_t scan_idx = 1; scan_idx < pointcloud_set.size() - 1 - 6000; ++scan_idx)
    {
        std::vector<Vector2d> target_scan = voxelGridDownsample(pointcloud_set[scan_idx], voxel_size);
