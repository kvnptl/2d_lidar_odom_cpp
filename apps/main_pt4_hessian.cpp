...
int main(int argc, char const *argv[])
{
    ...
    // Output the results
    std::cout << "[INFO] H matrix: \n"
              << H << std::endl;
    std::cout << "[INFO] b vector: \n"
              << b << std::endl;
    std::cout << "[INFO] Delta x: \n"
              << delta_x << std::endl;
    ...
    /*
    // Approach 2 (with more computational overhead, but easy to understand)
    // Construct the perturbation transformation matrix v2t(Δx)
    double delta_theta = delta_x[2];
    Matrix3d delta_T;
    delta_T << std::cos(delta_theta), -std::sin(delta_theta), delta_x[0],
        std::sin(delta_theta), std::cos(delta_theta), delta_x[1],
        0, 0, 1;

    // Current transformation matrix (initially identity)
    ...
    // Extract updated rotation matrix and translation vector
    R = T.block<2, 2>(0, 0);
    t = T.block<2, 1>(0, 2);
    */
    ...
    std::cout << "[INFO] Delta x: \n"
              << delta_x << std::endl;
    std::cout << "[INFO] Updated R: \n"
              << R << std::endl;
    std::cout << "[INFO] Updated t: \n"
