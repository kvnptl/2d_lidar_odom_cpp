...
std::vector<Vector2d> downsample(const std::vector<Vector2d> &points, double voxel_size)
{
    std::unordered_map<Eigen::Vector2i, std::vector<Vector2d>, Vector2iHash> voxel_map;
...
int main(int argc, char const *argv[])
{
    ...
    std::vector<Vector2d> source_scan = downsample(pointcloud_set[0], voxel_size);

    for (size_t scan_idx = 1; scan_idx < pointcloud_set.size() - 1 - 6000; ++scan_idx)
    {
        std::vector<Vector2d> target_scan = downsample(pointcloud_set[scan_idx], voxel_size);
        ...
        source_scan = downsample(source_scan, voxel_size);
