...
int main(int argc, char const *argv[])
{
    ...
    // Store all global transformations for visualization
    std::vector<Matrix2d> R_globals;
    std::vector<Vector2d> t_globals;

    // Store all local transformations for visualization
    std::vector<Matrix2d> R_locals;
    std::vector<Vector2d> t_locals;

    // Store all positions and orientations for visualization
    std::vector<Vector2d> positions;
    std::vector<double> orientations;
    positions.emplace_back(t_total); // Initial position
    orientations.emplace_back(0.0);  // Initial orientation (assuming starting at 0 radians)

    for (size_t scan_idx = 0; scan_idx < pointcloud_set.size() - 1 - 3000; ++scan_idx)
    {
        ...
        R_globals.emplace_back(R_total);
        t_globals.emplace_back(t_total);

        R_locals.emplace_back(R);
        t_locals.emplace_back(t);

        // Store the current position
        positions.emplace_back(t_total);
        orientations.emplace_back(atan2(R_total(1, 0), R_total(0, 0))); // Extract angle from rotation matrix
    }

    std::ofstream trajectory_file("trajectory.txt");
    for (const auto &pos : positions)
    {
        trajectory_file << pos.x() << " " << pos.y() << "\n";
    }
    trajectory_file.close();

    // std::ofstream trajectory_file("trajectory_with_orientation.txt");
    // for (size_t i = 0; i < positions.size(); ++i)
    // {
    //     trajectory_file << positions[i].x() << " " << positions[i].y() << " " << orientations[i] << "\n";
    // }
    // trajectory_file.close();
    ...
    // // Visualize the accumulated transformation
    // // Create a visualizer
    // open3d::visualization::Visualizer visualizer;
    // visualizer.CreateVisualizerWindow("Point Cloud Viewer", 800, 600);

    // for (size_t i = 0; i < R_locals.size() - 1; ++i)
    // {
    //     std::vector<Vector2d> target_scan = pointcloud_set[i];
    //     std::vector<Vector2d> source_scan = pointcloud_set[i + 1];

    //     std::vector<Vector2d> transformed_source_scan;
    //     for (const auto &point : source_scan)
    //     {
    //         Vector2d transformed_point = R_locals[i] * point + t_locals[i];
    //         transformed_source_scan.emplace_back(transformed_point);
    //     }

    //     viewer::viewTwoClouds(target_scan, transformed_source_scan, visualizer);
